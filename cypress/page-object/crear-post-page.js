...
export class CrearPostPage{
  ...
  setExpert(excerpt){
    ...
    this.todoPage.clickBoton('body',2000)
    this.todoPage.clickBoton(".blue.link.fw4.flex.items-center.ember-view",1000)
    cy.get("a[href='#/posts/']").first().click({ force: true });
    cy.get('.gh-list-data.gh-post-list-title').first().click({force: true});
    
    this.todoPage.clickBoton('.post-settings',500);

    cy.get("textarea[name='post-setting-custom-excerpt']").should('have.value', excerpt)
  }
